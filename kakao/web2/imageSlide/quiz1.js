...
var curPage = document.querySelector("cur-page");
var lastPage = document.querySelector('last-page');
var photoNum = todayPhotos.length;
var pageSize = 3;
var curPageIdx = 0;
var lastPageIdx = Math.ceil(photoNum/pageSize)-1;
...
function update(){
    deletePhoto();
    deletePhoto();
    for(var i=curPageFirstIdx() ; i<=curPageLastIdx(); i++){
...
function curPageFirstIdx(){
    return curPageIdx*pageSize;
}

function curPageLastIdx(){
    var expected = curPageFirstIdx() + pageSize-1;
    return (expected < photoNum-1) ? expected : photoNum-1;
}
